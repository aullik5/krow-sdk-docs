...
| 判据 | 需要你注册什么 | 适用范围 |
|---|---|---|
| 承诺交付的产物**不在磁盘上** / 是 0 字节 | **不需要**任何注册 | **任何**扩展名，开箱即用 |
...
- **内建的"欠账 → 补做步"映射是推理域专属的。** 它按 `(欠账类别, 标签)` 派发补做步，而那张类别表的条目全部来自推理 / 研究任务族。**产物型任务（文档 / 演示文稿 / 图片 / 自定义格式）走到它面前对不上任何类别，因此不会触发自动补做步。**
- **今天对产物型任务真正生效的是止损那一档**：质量停滞（自报成功、但产物签名一直不变）会触发一次**纯减法**的换框架 —— 撤掉饱和路子的余步、把缺口写进观测，交给通用收敛动作**如实**收尾。
- **这一格正在补**，方向是让欠账明细带结构化的 `(类别, 标签)` 上行，并按 OCP 让领域侧注册自己的"类别 → 补做步"映射（与 `register_decision_contract` / `with_visual_adapter` 同一范式）。**在它落地之前，不要靠往 error_vector 上叠更多分量来"逼"系统修复** —— 多一条轴只会让它更早停下来，不会让它修得更好。
