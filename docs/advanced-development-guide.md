...
**这一刀切在行边界上**（2026-08-18 起）：不会把你的句子、括号或 ``` 代码围栏切成
两半，切完还会挂一条 `…（此处截断，完整手册用 lookup_tool_docs(section="workflow")
索取）` 的显式标记，让 LLM 知道自己看的是半份。代价是实际保留量会比上限略少
（回退找行边界最多让出 10%），**所以别按"正好写满 2000"来规划**。

在此之前是纯 `raw[:cap]` 硬切。实测当时现网 8 处截断 8 处落在行中间、6 处留下不配对
的括号，`pptx_studio` 还留下一个未闭合的 ``` —— 拼进聚合 prompt 后把排在它后面的所有
ACT 引导都吞进了那个代码块。如果你之前观察到"我的 hint 明明在预算内却像被忽略了"，
很可能是被上一个 ACT 的坏围栏吃掉了。
...
这一条决定你的手册在**执行期**能不能被完整读到，是三个要点里唯一影响送达的。

规划期看到的是完整手册，执行期不是 —— 每一步的 prompt 有独立的收口，与规划期送了多少无关。手册按长度和能否分节落进三档：

| 手册形态 | 执行期每步拿到 | 后果 |
|---|---|---|
| ≤ ~800 字 | **全文内联** | 恒达，LLM 不需要做任何动作 |
| 超长，且有 ≥2 个 `##` / `###` 小节 | 一行指针；卡住时给**目录** | LLM 能按节名取回正文 |
| 超长，但通篇没有小标题 | **只有一行指针** | 取回时只能整份拉，没有可点的节名 |

`SKILL.md` 是自由 markdown，不写小标题完全合法 —— 于是长 skill 默认落进最差那档。加几个 `##` 的成本远低于它买到的东西。

**不要为了挤进第一档去删内容。** 压到 800 字以下的代价通常是删掉受力的规则，那是拿完整性换配额，方向反了。第二档是给长手册准备的正常形态。

**手册怎么被取回**

执行期 LLM 用 `lookup_tool_docs` 按层拉正文。它单次有 8000 字上限，超了切尾并在末尾给出下一个起始位置，带着那个位置再调一次就从断点接着拿 —— 长手册是分几次取回的，不会因为一次拉不完就丢。

还有一条兜底：物化产物的抬头里写着你的 `SKILL.md` **原文路径**，而 skill 根目录在装载时已授权给路径沙箱（见「边界」最后一条）。所以 LLM 也可以直接 `read_file` 按行范围读原文。对通篇没有小标题的超长手册，这往往比按层拉更省事。
...
3. **领域轴**：确实需要专属处置 → 用 `register_domain_axis(轴名)` 自注册（SDK 门面，见 [`api-reference.md` §9.6](./api-reference.md#96-metacognition--配置决策脑三注册表)），再给这条轴配一个能处置它的执行器 —— 最短路是 `register_control_reflex(..., on="你的轴名", actuator=...)`。只加轴不加执行器 = "只有传感器没有执行器"，这是本仓吃过多次的半边墙。

> 轴到底登记上没有，用 `get_registry_snapshot()["axes"]` 查；触发器强度口径写错（自报越界被静默夹取）看同一张快照的 `magnitude_clamped`。
> ⚠️ 更深的决策契约登记（`DecisionSpec`）住在 runtime 内部（`modules.agent.progressive.decision_contract`），**只在 monorepo 内可用**，外部 SDK 装机拿不到那条 import —— 需要它通常意味着你要的其实是控制反射或 step 执行器。
...
### 10.10 能力边界：**可见 ≠ 能止损 ≠ 可修复**（注册之前先读这一节 · 2026-08-24）

读完 §10.1–10.9 之后最容易产生的一个误解是："我把领域信号接进去了，决策脑就会帮我把产物修好。" **不会。** 决策脑的能力是**三档**，注册不同的东西买到的是不同的档；把它们混成一档，你会对同一次运行给出错的验收结论。

| 档 | 你买到了什么 | 你要注册什么 |
|---|---|---|
| **可见** | 传感器读得到"缺陷已经发生"，缺口会进结案统计与决策请求块 | SituationContributor（§10.2）；缺陷在**磁盘上那份产物**里时另见 §10.10.2 |
| **能止损** | 系统能**停止空烧**、撤掉走不通的余步，并**带着缺口诚实收敛**（而不是假装完成） | ReframeProvider（§10.9；`make_reframe_plan(new_step=None)` 只撤不加） |
| **可修复** | 产物**真的变好** | 内建通用面**做不到** —— 见 §10.10.3 的边界与已知缺口 |

**这三档要分开说，也要分开验收。** 一次运行里"缺口被看见了、空烧停了、任务如实报了未达标"是**成功**（前两档都兑现了）；把它读成失败（"怎么还没修好"）、或者反过来报成"决策脑修好了产物"，都是把三档混成了一档。

#### 10.10.1 决策脑改的是"任务怎么跑"，不是"内容的字节"

所有决策契约动的都是**跑法**（`mutates="pace"`）：**System-1 绝不亲手写内容的一个字节。** 但这条约束比字面窄 —— 它**不**等于"产物不会因为一条决策而不同"：

- 一条决策可以**让新工作发生**（重规划 / 换框架 / 补一步 / 继续），产物因此不同；
- 一条决策可以**把手里已经生成好的东西落到磁盘上**（物化 / 冲刷草稿 / 抢救最小交付）—— 内容逐字节取自已经生成的结果，系统一个字都不生成；
- 一条决策可以**换掉生成者**，或**让没跑完的跑完**（加预算 / 导航到未执行的交付步）。

⇒ 写执行器时正确的心智模型是"**我让哪一步重新发生**"，不是"我把哪段文字改掉"。想直接改内容那是 ToolPlugin 的活（§3），不是执行面的活。

#### 10.10.2 让决策脑"看得见你的产物"：质量轴是 **provider 注册表**，不是一张枚举表

常被问的一个问题："核心的认知轴清单里没有我这个格式，是不是就接不上？" **不是。** 质量缺口（`quality_deficit`）不是写死的枚举轴，它背后是一张 **provider 注册表** —— 注册表决定它今天能看见什么。内建的产物完整性判据是**领域无关**的三条，一个字都不提任何具体格式：

| 判据 | 需要你注册什么 | 适用范围 |
|---|---|---|
| 产物登记过、后来**不在磁盘上**了 / 是 0 字节（跨机改名 / 抢救覆盖 / 误删） | **不需要**任何注册 | **任何**扩展名，开箱即用 |
| 工具**报告了成功、也给出了输出路径，但一个字都没写** | **不需要**任何注册 | **任何**扩展名，开箱即用 |
| 产物**打不开** | 该扩展名的 VisualAdapter | 你注册过的扩展名 |
| 产物**打开了，但里面的单元是空的**（既无文字也无媒体） | 该扩展名的 VisualAdapter | 你注册过的扩展名 |

头两条是**两张发票**，别装回同一个信封（2026-08-25）：盘上读数一模一样（不在），来路、披露文案与重做对象都不同，所以引擎按两个缺口类别分别记账、分别限次。**"从头到尾没写过"曾经整个看不见** —— 产物登记表只收本地 `exists` 为真的路径（这道门是对的：登记表的消费方都默认"册子里的东西真实存在"），于是零产出的步骤连一条记录都不留。现在它记进**另一本**台账（已承诺未兑现），由这条轴在读的那一刻与磁盘对账；重做派给的就是当初承诺了这个路径的那一步。

所以，想让决策脑看见"我交出去的那份 `.dwg` / 自定义格式其实是空的"，你要做的只是按 [`api-reference.md` §2.6](./api-reference.md#26-visual-adapter-注入3-路径) 注册一个 VisualAdapter：

```python
agent = (
    AgentBuilder().from_env()
    .with_visual_adapter(".dwg", CADVisualAdapter)   # 一次注册，两份收益：视觉质检 + 产物完整性可见
    .build()
)
```

**没注册 adapter 的扩展名 = 这条轴对它合法休眠**，不会报假警（不会把每一份 `.md` 交付都判成缺陷）。

> **可迁移的判定口诀（不只对本章）**：问"这套系统能不能看见 X"的时候，把**注册表的每一个已注册项**点一遍，不要只数枚举常量。**枚举常量表记的是"当初写死了哪些"，注册表记的是"今天实际有哪些"。** 同样的关系也适用于 `register_decision_contract`（决策词表）与 document parser 注册表。

#### 10.10.3 修复面的能力边界（**已知缺口 · 在建**）

坦白讲清楚，免得你把时间花在期待一件今天不会发生的事上：

- **内建那张"欠账类别 → 补做步"映射表仍然是推理域专属的**（条目全部来自推理 / 研究任务族）。产物型任务对不上表里任何一个类别 —— 但这**不再**意味着它们拿不到补做步：2026-08-25 起产物缺口走的是**另一条来路**，绕过那张表。缺口类别由 provider 自己持有，处置侧只问"有没有形状"。
- **产物缺口的补做步派给谁，是确定性查表出来的**：重做一份坏产物的工具 = 当初产出（或**承诺产出**）它的那个工具，从引擎的两本台账里查 `tool_name` / `step_id`，原步的 `output_path` 原样带回。**查不到产出方就不派**（System-1 不猜工具）。
- **派出去 ≠ 修好。** 重做交给的是同一个工具、同一类 LLM 调用，**完全可能第二次照样零产出**。系统保证的只有三件事：空烧有界（同类缺口重注入有上限）、缺口不会因为重做过一次就自我宣布干净、披露里点得出是**哪一份**产物出了什么事。把"派出了补做步"读成"产物已修好"，就是把三档混成一档。
- **另一档照旧生效**：质量停滞（自报成功、但产物签名一直不变）会触发一次**纯减法**的换框架 —— 撤掉饱和路子的余步、把缺口写进观测，交给通用收敛动作**如实**收尾。
- **不要靠往 error_vector 上叠更多分量来"逼"系统修复** —— 多一条轴只会让它更早停下来，不会让它修得更好。

**现在就能做的替代**：把补做逻辑写成你自己的 **StepActuator**（§10.9）—— 它的适用性由你自判，不经过内建那张推理域类别表。

#### 10.10.4 你的执行器会撞上的两堵墙（**行为契约，不是 bug**）

1. **同指纹的重派会被静默判成已完成。** 引擎按 `tool` + 排序后的 `constraints` 给步骤算指纹（LLM 生成步另外算上 `purpose` 的开头一段），与已成功步同指纹的新步会被直接算作完成。⇒ **只改 `description` 的措辞不产生任何区别**；想让一步"真的重来"，必须换工具或换 `constraints`（例如把"这次别丢图"写成一条 constraint，而不是写进描述文案）。
2. **已执行的步撤不掉。** 撤步过三条不变量（已执行不撤 / 交付步不撤 / 被依赖步不撤），动作面**没有撤销键**，只有覆盖重写。⇒ 别设计依赖"回滚到第 3 步之前"的方案。

#### 10.10.5 慢环学到的东西不会让决策脑变聪明

§9 的睡眠蒸馏 / overlay 影响的是**任务 prompt**，不进决策请求块。⇒ 蒸馏出来的教训会改变 agent 干活的方式，**不会**改变决策脑何时唤醒、选哪个动作。想影响决策，走的是本章的三注册表 + 执行面，不是 overlay。

### 10.11 决策请求块里到底装了什么，以及它有多大（0.9.1.13）

唤醒那一拍，LLM 看到的不是一句"你卡住了"，而是两个块。你注册的东西最终就落在这两个块里，所以值得知道它们的形状和上限：

| 块 | 上限（字符） | 装什么 |
|---|---|---|
| 态势块 | `SITUATION_RENDER_MAX_CHARS` = 2100 | 用户终极目标重锚 / 各轴当前读数与趋势（含你的 `error_vector` 分量）/ 唤醒事由 / 能力菜单 / 改道提案 |
| 决策块 | `DECISION_RENDER_MAX_CHARS` = 4200 | 可选动作与约束视图 / **历史决策链**（run-compressed + 循环告警）/ 工具足迹 / 上一拍预期的结算（confirmed / violated）/ 加时是否真买回产出 |

两块都走统一的注入仲裁池（`INJECTION_BUDGET_CHARS` = 7800）。要点，按会咬到你的顺序：

1. **决策脑那两块是 `PRIORITY_CRITICAL`，无条件放行** —— 但它们占掉的字数照样从池子里扣。⇒ 你自己的 hint / provider 是被挤掉的那一方，而不是反过来。
2. **超上限不是报错，是按可裁剪度从尾部往前裁**。协议槽（改道提案、能力菜单这类"LLM 必须能照着做"的行）排在最前、最后被裁。你的唤醒事由字符串排在可裁区 —— **写长了不会更被重视，只会把自己挤到裁剪线以下**。一句话说清"什么现象 + 建议往哪走"即可，参 §10.3 的范例长度。
3. **历史决策链是决策脑自己的记忆**（0.9.1.13 起真正进块，此前只落台账不进 prompt）：连着三拍拍同一个动作、净产出为零，会带一条循环告警一起进块。⇒ 你不需要（也不该）在自己的 contributor 里重造"我劝过几次了"的计数，那份记账引擎已经在做。
4. **加时结算也进块**：`extend_budget` 之后那一拍的净产出会被结算，连续几次买不回产出会直接写成"别再第三次加时"。⇒ 想让决策脑停止空烧，靠的是让缺口读数**真实反映零进展**，而不是靠再多喊一次。

> 排查"我的信号明明注册了、块里却看不见"时：先看它是不是被裁在线下（块尾是否有截断标记），再看 `applicable()` 那一拍是否返 True。两者都排除了才去怀疑注册表（`get_registry_snapshot()`）。
